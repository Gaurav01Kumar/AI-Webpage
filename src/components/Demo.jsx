    ...
    <section className="mt-16 w-full max-w-xl">
        ...
        >
         ...
         ml-3 w-5"
          ...
          hover:text-gray-700 absolute
           inset-y-0 right-0 my-1.5 mr-1.5
          ...
          {allArticle.map((item, index) => {
            ...
            >
              <div className="w-7 h-7 rounded-full
               ...
               >
              <img src={copied===item.url?tick:copy} className="w-[40%] object-contain" />
              </div>
      ...
      justify-center items-center">
      {
        isFetching?(
          ...
          <p className="font-inter font-bold text-black">Well , that wasn't supposed to happen..
           <br/>
           <span className="font-satoshi font-normal">{error?.data?.error}</span>
          </p>
        ):(
          article.summary&&(
            <div className="flex flex-col gap-3">
              <h2 className="font-satoshi font-bold
              text-gray-600 text-xl">Article 
              <span className="font-black bg-gradient-to-r
              ...
              <div className="rounded-xl border border-gray-200 bg-white/20 shadow-[inset_10px_-50px_94px_0_rgb(199, 199, 199, 0.2)] backdrop-blur p-4">
                <p className="font-inter font-meduim text-sm text-gray-700">{article.summary}</p>
              </div>
            </div>
          )
        )
